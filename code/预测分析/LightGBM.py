...
def grid_search_tuning(X_train_val, y_train_val):
    ...
    print('\n' + '=' * 60)
    print('LightGBM 超参数寻优（网格搜索 + 5折交叉验证）')
    print('=' * 60)
    ...
    grid_search = GridSearchCV(
        base_model, PARAM_GRID, cv=kf, scoring=scoring,
    ...
    results['learning_rate'] = grid_search.cv_results_['param_learning_rate']
    results['num_leaves'] = grid_search.cv_results_['param_num_leaves']
    ...
    print(f'  参数: learning_rate={results.loc[best_rmse_idx, "learning_rate"]}, '
          f'num_leaves={results.loc[best_rmse_idx, "num_leaves"]}, '
          f'n_estimators={results.loc[best_rmse_idx, "n_estimators"]}')
    print(f'MAE最优: MAE={results.loc[best_mae_idx, "MAE"]:.4f}')
    print(f'  参数: learning_rate={results.loc[best_mae_idx, "learning_rate"]}, '
          f'num_leaves={results.loc[best_mae_idx, "num_leaves"]}, '
          f'n_estimators={results.loc[best_mae_idx, "n_estimators"]}')
    print(f'R2最优: R2={results.loc[best_r2_idx, "R2"]:.4f}')
    print(f'  参数: learning_rate={results.loc[best_r2_idx, "learning_rate"]}, '
          f'num_leaves={results.loc[best_r2_idx, "num_leaves"]}, '
          f'n_estimators={results.loc[best_r2_idx, "n_estimators"]}')
    ...
    # 返回以RMSE为准的最优参数
    best_params = {
        'learning_rate': results.loc[best_rmse_idx, 'learning_rate'],
        'num_leaves': results.loc[best_rmse_idx, 'num_leaves'],
...
def plot_tuning_curves(results):
    ...
    fig.suptitle('LightGBM超参数寻优曲线（5折交叉验证）', fontsize=18, y=0.98)
    ...
    fig.suptitle('LightGBM超参数寻优指标对比（5折交叉验证）', fontsize=18, y=0.98)
...
def train_model(X_train, y_train, X_val, y_val, best_params=None):
    if best_params:
        model = lgb.LGBMRegressor(**best_params, random_state=42, verbose=-1, n_jobs=-1)
    else:
...
def plot_results(y_true, y_pred, dates, residuals, model):
    ...
    ax.set_title('LightGBM: 实际AQI vs 预测AQI', fontsize=14)
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title('LightGBM: 残差分析', fontsize=14)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title('LightGBM: 实际值 vs 预测值散点图', fontsize=14)
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.barh([FEATURE_COLS[i] for i in sorted_idx], importances[sorted_idx], color='#8b5cf6')
    ax.set_xlabel('特征重要性', fontsize=12)
    ax.set_title('LightGBM: 特征重要性', fontsize=14)
...
def save_model(model, best_params):
    """保存训练好的LightGBM模型供Web端预测"""
    joblib.dump(model, OUTPUT_MODEL_DIR / 'lightgbm.pkl')
    meta = {
    ...
    with open(OUTPUT_MODEL_DIR / 'lightgbm_meta.json', 'w', encoding='utf-8') as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)
    print(f'LightGBM模型已保存至: {OUTPUT_MODEL_DIR}')
